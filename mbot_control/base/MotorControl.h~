...
FloatFunc rs232_temperature_callback;
void set_temperature_callback(FloatFunc i_func){
	rs232_temperature_callback = i_func;
}*/
...
static void mcssl_callback(int id, uint8_t *buf, int length)
{
    ...
    //rs232_temperature_callback(26.5f);
...
void mcssl_vaccum(bool yn){
    if(yn){
    	cssl_putchar(serial,'O');
    }else{
	cssl_putchar(serial,'X');
    }
...
void mcssl_base_move(int north_len){
   printf("(Base Node) north_len = %d\n",north_len);
   cssl_putchar(serial,'G');
   cssl_putchar(serial,north_len);
   cssl_putchar(serial,'F');
